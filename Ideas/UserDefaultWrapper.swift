...
/// A simple non subscribing wrapper for a value stored in UserDefaults
@propertyWrapper
...
public extension UserDefault {
    ...
    @usableFromInline
    internal init(defaultValue: Value, key: String, store: UserDefaults) {
        get = {
...
public extension UserDefault where Value: ExpressibleByNilLiteral {
    ...
    @usableFromInline
    internal init<T>(wrappedType: T.Type, key: String, store: UserDefaults) {
        get = {
            let value = store.object(forKey: key) as? Value
            return value ?? nil
        }
...
public extension UserDefault {
    ...
    @usableFromInline
    internal init<R>(key: String, store: UserDefaults) where Value == R?, R: RawRepresentable {
        get = {
